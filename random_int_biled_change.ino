...
void loop(){
    ...
    if(rng == 1){
        ...
        IRTimer = 5;                  //need to find a better way to 
time it
        Serial.print(rng);
        delay(50);
        rng = random(0,2);
...
/*    //controls magnet 2(Reactive actions)
    ...
    if (SendMag > 0 && SendMag < 10){
        digitalWrite(Mag2Push,HIGH);  //runs if signal lasted less than 
10 ms but more than 0
        digitalWrite(Mag2Pull,LOW);
        SendMag = 0;
    }
    if (SendMag > 10){
        digitalWrite(Mag2Push,LOW);   //runs if signal lasted longer 
than 10 ms
